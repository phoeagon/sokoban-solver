...
1. The following libraries are needed：`sys`，`collections`，`numpy`，`heapq`，`time`.
2. (`numpy` requires separate installation, while the other ones are installed with a default Python installation).

3. Run `sokoban.py` in your local environment.
...
`-l`：There are several set of maps included. Include ".txt" in the parameter. 
`-m`：Valid options include `bfs`，`dfs`，`ucs`, and `astar`.
...
```
The first line indicates the action taken by the actor，where `u`，`d`，`l`，`r indicate up, down, left, or right movements，
The corresponding capital letter indicates the movement in corresponding direction WHILE PUSHING AGAINST THE BOS.
The second line includes the total run time.

<a id="1"></a>
## 1. Overview

Sokoban is the [Box Pusher Game](https://www.mathsisfun.com/games/sokoban.html)，where a player moves acrosds a map and pushes
boxes to designated spots on the map.
...
Where `#` represent the wall，`.` is the destination(s)，`B` is the box(es）（`X` indicates that it's already in place),
`&` is the actor/protagonist（`%` if overlaps with one of the destination），and whitespaces are unoccupied spaces the actor can move into.
